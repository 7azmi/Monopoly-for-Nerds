    ...
    { 
        static Engine()
        {
            ...
            OnDiceRolled += times =>
            {
                if (times > 2)
                {
                    Platform.Log("Player rolled double thrice, go to jail you cheater!");
                    WhoseTurn.GoJail();
                }
                else
                {
        ...
        {
            Platform.Log($"{player.GetName()} ruthlessly smashed all his opponents and became the boss capital! ");
            Platform.Log($"No pity for losers. lol ");
            Thread.Sleep(5000);
            //Log($"re open  ");
        }
        ...
        {
            Platform.Log($"you landed on {property.GetName()}");
